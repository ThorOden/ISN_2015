...
class MoleculeEdit(QTreeWidget):
    def __init__(self, parent=None):
        ...
        QObject.connect(self, SIGNAL('customContextMenuRequested(QPoint)'), self.contextMenu)
...
class AtomeItem(QTreeWidgetItem):
    ...
    def createChild(self):
        nouveau = AtomeItem(self.ATOME_TYPE[self.nature.currentText()], self.molecule, self.LIAISON_TYPE[self.liaison.currentText()], len(self.childs), self)
    ...
    def getDrawCode(self):
        ...
        r.append(DRAW_LANGUAGE["avance"])
        if self.atome.nom in ["O", "N"]:
